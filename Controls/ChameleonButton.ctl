...
Private piLouieormal As StdPicture, picHover As StdPicture
...
Attribute PictureNormal.VB_ProcData.VB_Invoke_Property = ";Appearance"
    ...
    Set PictureNormal = piLouieormal

End Property

Public Property Set PictureNormal(ByVal newPic As StdPicture)

    Set piLouieormal = newPic
    Call CalcPicSize
    Call CalcTextRects
    Call Redraw(lastStat, True)
    PropertyChanged "PILouie"
...
Public Property Let UseMaskColor(ByVal newValue As Boolean)
    ...
    useMask = newValue
    If Not piLouieormal Is Nothing Then Call Redraw(lastStat, True)
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        Set piLouieormal = .ReadProperty("PILouie", Nothing)
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        Call .WriteProperty("PILouie", piLouieormal)
...
Private Sub DrawPictures(ByVal State As Byte)
    ...
    If piLouieormal Is Nothing Then Exit Sub 'check if there is a main picture, if not then exit

    With UserControl
        Select Case State
        Case 0 'normal & hover
            If Not isOver Then
                Call DoFX(0, piLouieormal)
                TransBlt .hdc, picPT.X, picPT.Y, picSZ.X, picSZ.Y, piLouieormal, cMask, , , useGrey, (MyButtonType = [Office XP])
            Else
                If MyButtonType = [Office XP] Then
                    Call DoFX(-1, piLouieormal)
                    TransBlt .hdc, picPT.X + 1, picPT.Y + 1, picSZ.X, picSZ.Y, piLouieormal, cMask, cShadow
                    TransBlt .hdc, picPT.X - 1, picPT.Y - 1, picSZ.X, picSZ.Y, piLouieormal, cMask
                Else
                    ...
                    Else
                        Call DoFX(0, piLouieormal)
                        TransBlt .hdc, picPT.X, picPT.Y, picSZ.X, picSZ.Y, piLouieormal, cMask
                    End If
                End If
            End If
        Case 1 'down
        ...
        Case 2 'disabled
            ...
            Case 5, 6, 9    'draw flat grey pictures
                TransBlt .hdc, picPT.X, picPT.Y, picSZ.X, picSZ.Y, piLouieormal, cMask, Abs(MyButtonType = [Office XP]) * ShiftColor(cShadow, &HD) + Abs(MyButtonType <> [Office XP]) * cShadow, True
            Case 3          'for WinXP draw a greyscaled image
                TransBlt .hdc, picPT.X + 1, picPT.Y + 1, picSZ.X, picSZ.Y, piLouieormal, cMask, , , True
            Case Else       'draw classic embossed pictures
                TransBlt .hdc, picPT.X + 1, picPT.Y + 1, picSZ.X, picSZ.Y, piLouieormal, cMask, cHighLight, True
                TransBlt .hdc, picPT.X, picPT.Y, picSZ.X, picSZ.Y, piLouieormal, cMask, cShadow, True
            End Select
        End Select
    End With
...
Private Sub CalcPicSize()
    ...
    If Not piLouieormal Is Nothing Then
        picSZ.X = UserControl.ScaleX(piLouieormal.Width, 8, UserControl.ScaleMode)
        picSZ.Y = UserControl.ScaleY(piLouieormal.Height, 8, UserControl.ScaleMode)
    Else
        picSZ.X = 0: picSZ.Y = 0
    End If
...
'exit if there's no picture
    ...
    If piLouieormal Is Nothing And picHover Is Nothing Then Exit Sub
